...
//! ## Opt-in only
//!
//! This test is **gated on `FERRITIN_SEARCH_REQUIRE_ORACLE=1`** and skips
...
//!
//! - `FERRITIN_SEARCH_QUERY_FASTA`: queries (default: vendored 50-seq
//!   fixture).
//! - `FERRITIN_SEARCH_TARGET_FASTA`: targets (default: same as queries —
...
//! - `FERRITIN_SEARCH_EXAMPLE_FASTA`: legacy alias setting both query and
//!   target to the same file.
//! - `FERRITIN_SEARCH_MMSEQS_BIN`: explicit path to the `mmseqs` binary
...
#[test]
fn ferritin_search_recall_matches_upstream() {
    // OPT-IN ONLY. cargo test must not pay this cost by default.
    if !require_oracle() {
        eprintln!(
            "SKIP ferritin_search_recall_matches_upstream: \
             set FERRITIN_SEARCH_REQUIRE_ORACLE=1 to run (test invokes upstream \
