    ...
    def build(
        ...
        *,
        ...
        if not rust_msa_available():
            raise RuntimeError(
                "Rust MSA backend is not available — install ferritin_connector "
                "(maturin develop --release in ferritin-connector/)"
            )
