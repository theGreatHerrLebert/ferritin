...
use ferritin_align::core::types::{Coord3D, MolType, StructureData};
...
pub fn extract_chains_for_alignment(
    pdb: &pdbtbx::PDB,
) -> Result<Vec<ferritin_align::ext::mmalign::ChainData>> {
    use ferritin_align::ext::mmalign::ChainData;
...
mod tests {
    ...
    fn test_save_roundtrip() {
        ...
        let tmp_path = "/tmp/ferritin_test_roundtrip.pdb";
    ...
    fn test_extract_for_alignment_mol_type() {
        ...
        let sd = extract_for_alignment(&pdb, None).expect("extract");
        assert!(
            matches!(sd.mol_type, ferritin_align::core::types::MolType::Protein),
