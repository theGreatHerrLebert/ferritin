...
#[pyfunction]
#[pyo3(signature = (pdb, probe=1.4, n_points=960))]
pub fn atom_sasa<'py>(
    py: Python<'py>,
    pdb: &PyPDB,
    probe: f64,
    ...
    let result = py.allow_threads(|| sasa::sasa_from_pdb(&pdb.inner, probe, n_points));
...
pub fn residue_sasa<'py>(
    ...
    let atom_areas =
        py.allow_threads(|| sasa::sasa_from_pdb(&pdb.inner, probe, n_points));
...
pub fn total_sasa(
    ...
    let areas = py.allow_threads(|| sasa::sasa_from_pdb(&pdb.inner, probe, n_points));
    areas.iter().sum()
}
...
#[pyfunction]
#[pyo3(signature = (structures, probe=1.4, n_points=960, n_threads=None))]
pub fn batch_atom_sasa<'py>(
    ...
    // Extract coords + radii on main thread
    let data: Vec<(Vec<[f64; 3]>, Vec<f64>)> = structures
        ...
        .map(|item| {
            ...
            for atom in pdb.inner.atoms() {
                ...
                let r = atom
                    .element()
                    .and_then(|e| sasa::vdw_radius(e.symbol()))
                    .unwrap_or(sasa::DEFAULT_RADIUS);
                radii.push(r);
            }
            Ok((coords, radii))
        })
...
#[pyfunction]
#[pyo3(signature = (paths, probe=1.4, n_points=960, n_threads=None))]
            ...
            path_strs
                ...
                .filter_map(|(i, path)| {
                    permissive_load(path).ok().map(|pdb| {
                        let areas = sasa::sasa_from_pdb(&pdb, probe, n_points);
                        (i, areas.iter().sum())
                    })
                })
                .collect()
