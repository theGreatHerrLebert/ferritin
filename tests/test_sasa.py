...
class TestAtomSASA:
    ...
    def test_total_reasonable(self):
        ...
        s = load_crambin()
        total = ferritin.atom_sasa(s).sum()
    ...
    def test_different_n_points(self):
        ...
        s = load_crambin()
        sasa_100 = ferritin.atom_sasa(s, n_points=100).sum()
        sasa_960 = ferritin.atom_sasa(s, n_points=960).sum()
    ...
    def test_probe_radius_effect(self):
        ...
        s = load_crambin()
        sasa_14 = ferritin.atom_sasa(s, probe=1.4).sum()
        sasa_20 = ferritin.atom_sasa(s, probe=2.0).sum()
...
class TestTotalSASA:
    def test_matches_atom_sum(self):
        s = load_crambin()
        total = ferritin.total_sasa(s)
        atom_sum = ferritin.atom_sasa(s).sum()
        np.testing.assert_allclose(total, atom_sum, rtol=1e-10)

    def test_crambin_range(self):
        total = ferritin.total_sasa(load_crambin())
...
class TestBiopythonOracle:
    ...
    def test_total_sasa_matches_biopython(self, crambin_biopython_sasa):
        """Ferritin total SASA should be within 2% of Biopython."""
        s = load_crambin()
        ferritin_total = ferritin.total_sasa(s)
        bp_total = crambin_biopython_sasa
        rel_diff = abs(ferritin_total - bp_total) / bp_total
        assert rel_diff < 0.02, (
            f"Ferritin {ferritin_total:.1f} vs Biopython {bp_total:.1f} "
...
class TestBatchSASA:
    ...
    def test_batch_matches_serial(self):
        structures = [load_crambin(), load_ubiquitin()]
        totals_batch = ferritin.batch_total_sasa(structures, n_threads=-1)
        totals_serial = np.array([ferritin.total_sasa(s) for s in structures])
...
class TestLoadAndSASA:
    def test_basic(self):
        ...
        results = ferritin.load_and_sasa(paths, n_threads=1)
